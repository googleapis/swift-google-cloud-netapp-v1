# Google Cloud Client Libraries for Swift - NetApp API
...
Add `swift-google-cloud-netapp-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-netapp-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudNetAppV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudNetAppV1 <target-name> --package swift-google-cloud-netapp-v1
```
